            ...
            foreach ($emails as $uid) {
                ...
                if ($email->property_id && !$isInternalEmail) {
                    ...
                    if ($direction === 'inbound' && strlen($body) > 100) {
                        try {
                            // DISABLED: $this->autoExtractFromEmail($email->id, $email->property_id, $body, $subject);
                        } catch (\Throwable $e) {
                    ...
                    try {
                        ...
                        $conversation = $convService->updateFromEmail($email, $newActivity ?? null);
                    } catch (\Throwable $e) {
                    ...
                    if ($direction === 'inbound' && in_array($finalCategory, $draftCategories) && isset($newActivity)) {
                        try {
                            // DISABLED: $this->autoGenerateDraft($newActivity->id, $email->property_id, $stakeholder, $email->id);
                        } catch (\Throwable $e) {
                    ...
                    if (false && $direction === 'inbound' && $finalCategory === 'anfrage' && $email->property_id && isset($newActivity)) {
                        try {
                            // DISABLED: $this->autoReplyToErstanfrage($email, $newActivity, $stakeholder, $account);
                        } catch (\Throwable $e) {
...
$system = 'Du analysierst Kaufanbot-Aktivitäten einer Immobilie und erstellst eine Statusübersicht. Antworte NUR als JSON mit diesem Format: {"sold_units": [{"unit": "Top X", "buyer": "Name", "date": "YYYY-MM-DD", "status": "unterschrieben|gegengezeichnet|in Bearbeitung", "details": "kurze Info"}], "summary": "Zusammenfassung in 1-2 Saetzen"}';
    ...
    private function autoReplyToErstanfrage($email, $activity, string $stakeholder, $account): void
    {
        ...
        if (!$settings || empty($settings->auto_reply_enabled)) {
        ...
        try {
            ...
            if ($status === 'sent') {
                try {
                    $convEmail = strtolower($realEmail ?? $email->from_email);
                    $conv = \App\Models\Conversation::where('contact_email', $convEmail)
                        ->where('property_id', $email->property_id)
                        ->first();
                    if ($conv) {
                        $conv->update([
                    ...
                    \Log::warning("ConversationService auto-reply update failed: " . $e->getMessage());
                }
            }
    ...
    {
        // Strip quoted/forwarded text to avoid extracting knowledge from our own replies
        $cleanBody = strip_tags($body);
        // Remove lines starting with > (classic quote markers)
        $cleanBody = preg_replace('/^>.*$/m', '', $cleanBody);
        // Remove everything after common reply delimiters
        $delimiters = [
        ...
        foreach ($delimiters as $d) {
            $pos = mb_stripos($cleanBody, $d);
            if ($pos !== false && $pos > 50) {
                $cleanBody = mb_substr($cleanBody, 0, $pos);
                break;
            }
        }
        $cleanBody = trim($cleanBody);
        if (mb_strlen($cleanBody) < 30) {
            Log::info("autoExtractFromEmail: skipping email {$emailId} - too short after stripping quotes");
            return;
        }
        $snippet = mb_substr($cleanBody, 0, 1500);
...
Leeres Array [] wenn keine NEUEN Fakten enthalten sind.';
        ...
        foreach ($entries as $entry) {
            ...
            // Skip person-specific feedback — causes AI to attribute one person's feedback to others
            if (in_array($category, ['feedback_besichtigung', 'feedback_negativ', 'feedback_positiv'])) continue;
