        ...
        foreach ($folders as $folderInfo) {
            ...
            foreach ($emails as $uid) {
                ...
                if ($email->property_id && !$isInternalEmail) {
                    ...
                    // Save auto-generated draft to conversation
                    if (isset($conversation) && $conversation) {
                        try {
                            $latestDraft = \DB::table('email_drafts')
                                ->where('source_email_id', $email->id)
                                ->orderBy('id', 'desc')
                                ->first();
                            if ($latestDraft && $latestDraft->body) {
                                app(\App\Services\ConversationService::class)->saveDraft(
                                    $conversation,
                                    $latestDraft->body,
                                    $latestDraft->subject ?? null,
                                    $latestDraft->to_email ?? null
                                );
                            }
                        } catch (\Throwable $e) {
                            \Log::warning("ConversationService draft save failed: " . $e->getMessage());
                        }
                    }
