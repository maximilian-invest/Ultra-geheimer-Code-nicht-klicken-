...
async function delegateTask(task) {
  try {
    const r = await fetch(API.value + "&action=delegate_task", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ task_id: task.id }),
    });
    const d = await r.json();
    if (d.success) { toast("An Assistenz delegiert"); loadTasks(); }
...
<template>
    <div class="px-4 py-6 space-y-6">
        ...
        <!-- Tasks (hidden for assistenz, they have their own tab) -->
        <template v-if="false" >
        <div class="card">
            <div class="px-6 py-3 flex items-center justify-between border-b border-[var(--border)]">
                <h3 class="text-sm font-semibold flex items-center gap-2">
                    ...
                    <div v-for="task in visibleTasks" :key="task.id">
                        ...
                        <div v-if="editingTask?.id !== task.id" class="flex items-center gap-2 py-2 px-2 rounded-lg hover:bg-[var(--accent)] transition-colors group">
                            ...
                            <div class="flex gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                ...
                                <button @click="delegateTask(task)" class="btn btn-ghost btn-sm" style="color:#8b5cf6;font-size:10px" title="An Assistenz delegieren">Assistenz</button>
                            </div>
                        </div>
                        <!-- Edit mode inline -->
